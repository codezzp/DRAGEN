...
def encode_jsonl(
    *,
    ...
    if not rows:
        raise SystemExit(f"No text rows found: {input_jsonl}")
    rows.sort(key=lambda item: item[0])
    keys = [key for key, _ in rows]
    texts = [text for _, text in rows]
    embs: List[np.ndarray] = []
    with torch.no_grad():
        for start in tqdm(range(0, len(texts), batch_size), desc=f"encode {key_field}"):
    ...
    write_json(
        out_meta,
        {
            **expected_meta,
            ...
            "source_file_hash": sha256_file(input_jsonl),
            "created_at": utc_now(),
        },
    )
